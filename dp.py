...
def dp_histogram(epsilon):
  sensitivity = 1 # count changes by 1 for each neighboring dataset
  mu = 0
  ...
  for (age, music, value) in rows:
    # compute the noised value + round the noised_value to the closest integer.
    noised_value = int(round(value + laplace(mu, b), 0))
